...
### 2.3 벡터 검색 엔진 (Workspace AI Semantic Search)
Workspace 탭 통합 검색을 위해 기존 단순 키워드 검색을 넘어 자연어(문맥) 기반 검색 및 다중 필터링을 제공합니다.
...
*   **다중 필터링 및 복합 쿼리**: `pgvector` 기반 코사인 유사도 점수와 카테고리, 상태, 날짜 필터를 조합하여 최적의 검색 결과를 도출합니다.
*   **상태 기반 검색 제한**: DRAFT(검증 대기) 상태의 데이터는 검색 결과에서 배제하며, 오직 `PUBLISHED`(승인 완료) 상태의 지식만 일반 사용자에게 노출합니다.

### 2.4 Admin 대시보드 및 통계 (Admin Dashboard & Analytics)
System Admin 탭에서 전체 시스템의 지표를 한눈에 파악할 수 있는 대시보드를 제공합니다.
*   **요약 지표 (Overview Stats)**: 총 문서 수, 상태별 문서 수 통계 제공.
*   **카테고리 통계 (Category Stats)**: 카테고리별 문서 분포 시각화를 위한 데이터 서빙.
*   **최근 활동 내역 (Recent Activities)**: 문서 등록 및 승인 등 최근 이벤트를 타임라인 형태로 트래킹하여 관리자에게 제공합니다.
...
### 2.6 테스트 인프라 전략 (Testing Infrastructure Strategy)
...
### 2.7 보안 아키텍처 원칙 (Secure Architecture Principles)
...
### 2.8 인프라 환경변수 무결성 (Infrastructure Environment Integrity)
...
### 2.9 데이터베이스 동기화 및 마이그레이션 (Database Migration Strategy)
